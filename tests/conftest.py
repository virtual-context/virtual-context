...
# ---------------------------------------------------------------------------
# Per-process embedding-model sharing
# ---------------------------------------------------------------------------
# Every engine construction owns its own EmbeddingProvider, and the lazy
# local load inside get_embed_fn builds a fresh SentenceTransformer per
# provider instance. Under the test suite that means every engine-building
# test pays a full torch model load, and a long-lived xdist worker ratchets
# to multiple GB of resident memory as allocator pools accumulate across
# hundreds of loads. One model per model_name per worker process is
# behavior-identical for tests (the model is stateless encode) and caps the
# worker's footprint at a single copy.
#
# Only SUCCESSFUL local loads are cached: disabled providers, injected
# embed_fn providers, and load-failure paths keep their exact per-instance
# semantics, so tests that pin those behaviors are unaffected.

@pytest.fixture(scope="session", autouse=True)
def _shared_embedding_model_per_process():
    from virtual_context.core import embedding_provider as _ep

    cache: dict[str, object] = {}
    original = _ep.EmbeddingProvider.get_embed_fn

    def cached_get_embed_fn(self):
        if self._disabled or self._loaded or self._load_failed:
            return original(self)
        hit = cache.get(self._model_name)
        if hit is not None:
            self._embed_fn = hit
            self._loaded = True
            return hit
        fn = original(self)
        if fn is not None:
            cache[self._model_name] = fn
        return fn

    _ep.EmbeddingProvider.get_embed_fn = cached_get_embed_fn
    try:
        yield
    finally:
        _ep.EmbeddingProvider.get_embed_fn = original

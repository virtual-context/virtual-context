...
Covers:
- EngineState fields (flushed_prefix_messages, last_request_time)
- SessionState persistence roundtrip
- history_offset behavior
- Flush gate logic in prepare_payload
- Invariant: flushed_prefix_messages <= compacted_prefix_messages
"""
...
from virtual_context.engine import _restored_flushed_prefix_messages
...
class TestEngineStateFields:
    def test_defaults(self):
        es = EngineState()
        assert es.flushed_prefix_messages == 0
        assert es.last_request_time == 0.0

    def test_assignment(self):
        es = EngineState()
        es.flushed_prefix_messages = 10
        es.last_request_time = 1234567890.0
        assert es.flushed_prefix_messages == 10
...
class TestEngineStateSnapshot:
    def test_snapshot_has_fields(self):
        snap = EngineStateSnapshot(
            ...
            compacted_prefix_messages=5,
            turn_tag_entries=[],
            turn_count=0,
            flushed_prefix_messages=3,
            last_request_time=100.0,
        )
        assert snap.compacted_prefix_messages == 5
        assert snap.flushed_prefix_messages == 3
    ...
    def test_snapshot_defaults(self):
        ...
        assert snap.flushed_prefix_messages == 0
...
class TestHistoryOffset:
    def test_basic_offset(self):
        es = EngineState(compacted_prefix_messages=5)
        assert es.history_offset(10) == 5

    def test_compacted_exceeds_history_returns_zero(self):
        es = EngineState(compacted_prefix_messages=20)
        assert es.history_offset(10) == 0

    def test_flushed_through_does_not_affect_history_offset(self):
        """history_offset uses compacted_prefix_messages, not flushed_prefix_messages."""
        es = EngineState(compacted_prefix_messages=5, flushed_prefix_messages=0)
        assert es.history_offset(10) == 5
...
class TestSessionStatePersistence:
    def test_roundtrip(self):
        original = SessionState(
            ...
            flushed_prefix_messages=7,
            last_request_time=1234567890.5,
        )
        serialized = original.to_json()
        restored = SessionState.from_json(serialized)
        assert restored.compacted_prefix_messages == 10
        assert restored.flushed_prefix_messages == 7
        assert restored.flushed_prefix_messages_present is True
        assert restored.last_request_time == 1234567890.5

    def test_backward_compat_missing_fields(self):
        """Old data without flushed_prefix_messages/last_request_time gets defaults."""
        old_data = json.dumps({
            "compacted_prefix_messages": 5,
            "last_compacted_turn": 3,
        }).encode()
        restored = SessionState.from_json(old_data)
        assert restored.flushed_prefix_messages == 0
        assert restored.flushed_prefix_messages_present is False
        assert restored.last_request_time == 0.0
        assert restored.compacted_prefix_messages == 5
...
class TestRestoreFlushedThrough:
    def test_missing_field_autosyncs_to_compacted(self):
        assert _restored_flushed_prefix_messages(10, 0, present=False) == 10

    def test_present_zero_is_preserved(self):
        assert _restored_flushed_prefix_messages(10, 0, present=True) == 0

    def test_present_value_is_preserved(self):
        assert _restored_flushed_prefix_messages(10, 4, present=True) == 4
...
class TestFlushGateLogic:
    ...
    The gate logic:
    - defer=False: flushed_prefix_messages auto-tracks compacted_prefix_messages
    ...
    def test_defer_off_auto_tracks(self):
        """When defer=False, flushed_prefix_messages should equal compacted_prefix_messages."""
        es = EngineState(compacted_prefix_messages=10, flushed_prefix_messages=0)
        # Simulate legacy auto-track (Step 5a)
        _defer = False
        _ct = es.compacted_prefix_messages
        _ft = es.flushed_prefix_messages
        if not _defer and _ct > _ft:
            es.flushed_prefix_messages = _ct
        assert es.flushed_prefix_messages == 10
    ...
    def test_cold_cache_flushes(self):
        ...
        if _should_flush_cold:
            es.flushed_prefix_messages = es.compacted_prefix_messages
        assert es.flushed_prefix_messages == 10
    ...
    def test_warm_cache_defers(self):
        ...
        _cache_age = time.time() - es.last_request_time
        _should_flush_cold = _cache_age >= _flush_ttl
        _flush_pending = es.compacted_prefix_messages > es.flushed_prefix_messages
        _warm_defer = _defer and not _should_flush_cold and _flush_pending

        assert not _should_flush_cold
        assert _flush_pending
        assert _warm_defer
        # flushed_prefix_messages should NOT change
        assert es.flushed_prefix_messages == 5
    ...
    def test_multiple_compaction_cycles_warm(self):
        ...
        # Simulate 3 compaction cycles advancing compacted_prefix_messages
        for new_ct in [8, 12, 15]:
            es.compacted_prefix_messages = new_ct
    ...
    def test_flushed_never_exceeds_compacted(self):
        """Invariant: flushed_prefix_messages should never exceed compacted_prefix_messages."""
        es = EngineState(compacted_prefix_messages=5, flushed_prefix_messages=0)
        # Normal: flush up to compacted
        es.flushed_prefix_messages = es.compacted_prefix_messages
        assert es.flushed_prefix_messages <= es.compacted_prefix_messages
        # Abnormal: if somehow flushed > compacted, it's a bug
        es.flushed_prefix_messages = 10
        assert es.flushed_prefix_messages > es.compacted_prefix_messages  # detects the invariant violation
...
class TestStorageBackendFields:
    def test_filesystem_roundtrip(self, tmp_path):
        """Filesystem backend preserves flushed_prefix_messages and last_request_time."""
        ...
        assert loaded.compacted_prefix_messages == 10
        assert loaded.flushed_prefix_messages == 7

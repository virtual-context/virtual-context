 ...
 WHERE c.phase = 'active'
   ...
   AND NOT EXISTS (
     ...
     HAVING SUM(CASE WHEN ct_untagged.tagged_at IS NULL THEN 1 ELSE 0 END) > 0
        AND (
            (COUNT(*) = 1
             AND SUM(CASE WHEN btrim(COALESCE(ct_untagged.user_content, '')) <> ''
                          THEN 1 ELSE 0 END) = 1
             AND SUM(CASE WHEN btrim(COALESCE(ct_untagged.assistant_content, '')) <> ''
                          THEN 1 ELSE 0 END) = 1)
            OR
            (COUNT(*) = 2
             AND SUM(CASE WHEN btrim(COALESCE(ct_untagged.user_content, '')) <> ''
             ...
             AND SUM(CASE WHEN btrim(COALESCE(ct_untagged.assistant_content, '')) <> ''
                          THEN 1 ELSE 0 END) = 1
             AND SUM(CASE WHEN btrim(COALESCE(ct_untagged.user_content, '')) <> ''
                                AND btrim(COALESCE(ct_untagged.assistant_content, '')) <> ''
                          THEN 1 ELSE 0 END) = 0)
            OR
            (COUNT(*) = 1
             ...
             AND (
                 SUM(CASE WHEN btrim(COALESCE(ct_untagged.user_content, '')) <> ''
                          THEN 1 ELSE 0 END)
                 + SUM(CASE WHEN btrim(COALESCE(ct_untagged.assistant_content, '')) <> ''
                            THEN 1 ELSE 0 END)
             ) = 1)
        )
   )
...
class PostgresStore(ContextStore):
    ...
    # Set only by ``save_attested_canonical_pair``. Both physical halves and
    # the immutable source-membership row must share one PostgreSQL
    # transaction; ordinary canonical writes continue to use their own
    # transaction exactly as before.
    _attested_pair_connection: contextvars.ContextVar = contextvars.ContextVar(
        "vc_attested_pair_connection", default=None,
    )
    # The outer reconcile transaction owns conversation_lifecycle's row lock.
    # Record the generation it proved so nested canonical writes on another
    # pooled connection can require that same monotonic lifecycle without
    # deadlocking by trying to reacquire the outer lock.
    _reconcile_generation_scope: contextvars.ContextVar = contextvars.ContextVar(
        "vc_reconcile_generation_scope", default=None,
    )
    ...
    @contextmanager
    def _canonical_write_transaction(self):
        active = self._attested_pair_connection.get()
        ...
        with self.pool.connection() as conn:
            ...
            with conn.transaction():
                ...
                if holder_timeout_s > 0:
                    ...
                    holder_timer.start()
                generation_token = self._reconcile_generation_scope.set(
                    (
                        conversation_id,
                        observed_generation,
                        bool(lifecycle["deleted"]) if lifecycle is not None else True,
                    )
                )
                try:
                    yield
                finally:
                    self._reconcile_generation_scope.reset(generation_token)
                    with holder_guard:
                        holder_active = False
    ...
    def _ensure_schema_locked(self) -> None:
        ...
        try:
            ...
            self._ensure_canonical_message_source_schema()
    ...
    def _ensure_canonical_message_source_schema(self) -> None:
        """Create the immutable transport-message membership ledger.

        ``canonical_turns`` owns serving order and may move between owners in
        VCMERGE.  A source membership instead keys the audience where the
        platform message was observed, so it survives an owner move without a
        rewrite.  No historical canonical row is backfilled here: old source
        columns are precisely the evidence under repair and are not authority.
        """
        with self.pool.connection() as conn:
            conn.execute(
                """CREATE TABLE IF NOT EXISTS canonical_message_sources (
                       ...
                       account_id TEXT NOT NULL,
                       message_id TEXT NOT NULL,
                       canonical_turn_id UUID NOT NULL UNIQUE
                           REFERENCES canonical_turns(canonical_turn_id)
                           ON DELETE CASCADE,
                       assistant_canonical_turn_id UUID
                           REFERENCES canonical_turns(canonical_turn_id)
                           ON DELETE CASCADE,
                       ...
                       CONSTRAINT canonical_message_sources_pair_shape
                       CHECK (
                           (pair_version = 1
                            AND assistant_canonical_turn_id IS NOT NULL
                            AND assistant_turn_hash <> '')
                       )
                   )"""
            )
            conn.execute(
                """ALTER TABLE canonical_message_sources
                   ADD COLUMN IF NOT EXISTS assistant_canonical_turn_id UUID"""
            )
            conn.execute(
                """ALTER TABLE canonical_message_sources
                   ADD COLUMN IF NOT EXISTS assistant_turn_hash TEXT
                   NOT NULL DEFAULT ''"""
            )
            conn.execute(
                """ALTER TABLE canonical_message_sources
                   ADD COLUMN IF NOT EXISTS turn_group_number INTEGER
                   NOT NULL DEFAULT -1"""
            )
            conn.execute(
                """ALTER TABLE canonical_message_sources
                   ADD COLUMN IF NOT EXISTS pair_version INTEGER
                   NOT NULL DEFAULT 1"""
            )
            conn.execute(
                """ALTER TABLE canonical_message_sources
                   ALTER COLUMN pair_version SET DEFAULT 1"""
            )
            ...
            if int(unsafe_pairs or 0):
                raise RuntimeError(
                    "canonical_message_sources contains unpaired legacy "
                    "memberships; exact-evidence repair is required"
                )
            conn.execute(
                """DO $$ BEGIN
                       IF NOT EXISTS (
                           SELECT 1 FROM pg_constraint
                            WHERE conrelid =
                                  'canonical_message_sources'::regclass
                              AND conname =
                                  'canonical_message_sources_assistant_fk'
                       ) THEN
                           ALTER TABLE canonical_message_sources
                           ADD CONSTRAINT
                               canonical_message_sources_assistant_fk
                           FOREIGN KEY (assistant_canonical_turn_id)
                           REFERENCES canonical_turns(canonical_turn_id)
                           ON DELETE CASCADE;
                       END IF;
                   END $$"""
            )
            # Replace, rather than merely create, because the first draft of
            # this schema coupled pair identity to turn_group_number. Group is
            # mutable placement (VCMERGE/resequencing); the durable pair is the
            # two row ids plus their immutable hashes.
            conn.execute(
                """ALTER TABLE canonical_message_sources
                   DROP CONSTRAINT IF EXISTS
                       canonical_message_sources_pair_shape"""
            )
            conn.execute(
                """ALTER TABLE canonical_message_sources
                   ADD CONSTRAINT canonical_message_sources_pair_shape
                   CHECK (
                       (pair_version = 1
                        AND assistant_canonical_turn_id IS NOT NULL
                        AND assistant_turn_hash <> '')
            ...
            conn.execute(
                """CREATE INDEX IF NOT EXISTS
                       idx_canonical_message_sources_turn
                   ON canonical_message_sources (canonical_turn_id)"""
            )
            conn.execute(
                """CREATE INDEX IF NOT EXISTS
                       idx_canonical_message_sources_channel
                   ...
                   BEGIN
                       IF (
                           EXISTS (
                               ...
                               OLD.turn_hash IS DISTINCT FROM NEW.turn_hash OR
                               OLD.hash_version IS DISTINCT FROM NEW.hash_version OR
                               OLD.normalized_user_text IS DISTINCT FROM NEW.normalized_user_text OR
                               OLD.normalized_assistant_text IS DISTINCT FROM NEW.normalized_assistant_text OR
                               ...
                               OLD.origin_channel_id IS DISTINCT FROM NEW.origin_channel_id OR
                               OLD.origin_channel_label IS DISTINCT FROM NEW.origin_channel_label OR
                           ...
                           EXISTS (
                               SELECT 1 FROM canonical_message_sources
                                WHERE assistant_canonical_turn_id =
                ...
                """CREATE OR REPLACE FUNCTION
                   ...
                   BEGIN
                       IF OLD.tenant_id IS DISTINCT FROM NEW.tenant_id OR
                          OLD.agent_scope_id IS DISTINCT FROM NEW.agent_scope_id OR
                          ...
                          OLD.canonical_turn_id IS DISTINCT FROM NEW.canonical_turn_id OR
                          OLD.assistant_canonical_turn_id IS DISTINCT FROM NEW.assistant_canonical_turn_id OR
                          OLD.assistant_turn_hash IS DISTINCT FROM NEW.assistant_turn_hash OR
                          OLD.turn_group_number IS DISTINCT FROM NEW.turn_group_number OR
                          OLD.pair_version IS DISTINCT FROM NEW.pair_version OR
                          OLD.audience_conversation_id IS DISTINCT FROM NEW.audience_conversation_id OR
                          OLD.channel_id IS DISTINCT FROM NEW.channel_id OR
                          OLD.guild_id IS DISTINCT FROM NEW.guild_id OR
                          OLD.author_id IS DISTINCT FROM NEW.author_id OR
                          OLD.source_actor_id IS DISTINCT FROM NEW.source_actor_id OR
                          OLD.transport_body_sha256 IS DISTINCT FROM NEW.transport_body_sha256 OR
                          OLD.canonical_body_sha256 IS DISTINCT FROM NEW.canonical_body_sha256 OR
                          OLD.projection_version IS DISTINCT FROM NEW.projection_version OR
                          OLD.canonical_turn_hash IS DISTINCT FROM NEW.canonical_turn_hash OR
                          OLD.reply_target_message_id IS DISTINCT FROM NEW.reply_target_message_id OR
                          OLD.created_at IS DISTINCT FROM NEW.created_at
                       THEN
                           RAISE EXCEPTION
                               'canonical source membership is immutable'
                               USING ERRCODE = 'integrity_constraint_violation';
                       END IF;
                       RETURN NEW;
                   END;
    ...
    def _assert_canonical_message_source_schema(self) -> None:
        ...
        with self.pool.connection() as conn:
            ...
            if table is None:
                raise RuntimeError(
                    "canonical_message_sources missing after schema bootstrap"
                )
            columns = {
                str(row["column_name"])
            ...
            primary = conn.execute(
                """SELECT pg_get_constraintdef(oid) AS definition
                     FROM pg_constraint
                    WHERE conrelid = 'canonical_message_sources'::regclass
                      AND contype = 'p'"""
            ).fetchone()
            expected_pk = (
                "PRIMARY KEY (tenant_id, agent_scope_id, platform, "
                "account_id, message_id)"
            )
            if primary is None or str(primary["definition"]) != expected_pk:
                raise RuntimeError(
                    "canonical_message_sources has an unsafe identity key"
                )
            assistant_fk = conn.execute(
                """SELECT pg_get_constraintdef(oid) AS definition
                     FROM pg_constraint
                    WHERE conrelid = 'canonical_message_sources'::regclass
                      AND conname =
            ...
            if (
                assistant_fk is None
                or str(assistant_fk["definition"])
                != "FOREIGN KEY (assistant_canonical_turn_id) REFERENCES "
                   "canonical_turns(canonical_turn_id) ON DELETE CASCADE"
            ):
                raise RuntimeError(
                    "canonical_message_sources assistant FK is missing or unsafe"
                )
            pair_shape = conn.execute(
                """SELECT 1 FROM pg_constraint
                    WHERE conrelid = 'canonical_message_sources'::regclass
                      AND conname = 'canonical_message_sources_pair_shape'
                      AND contype = 'c'"""
            ).fetchone()
            assistant_index = conn.execute(
                """SELECT indexdef FROM pg_indexes
                    WHERE tablename = 'canonical_message_sources'
                      AND indexname =
                          'idx_canonical_message_sources_assistant'"""
            ).fetchone()
            if (
                ...
                or "UNIQUE INDEX" not in str(assistant_index["indexdef"])
                or "WHERE (assistant_canonical_turn_id IS NOT NULL)"
                   not in str(assistant_index["indexdef"])
            ):
                raise RuntimeError(
                    "canonical_message_sources pair shape/index is unsafe"
                )
            unsafe_pairs = conn.execute(
                """SELECT COUNT(*) AS n FROM canonical_message_sources
                    WHERE pair_version <> 1
            ...
            if source_trigger is None:
                raise RuntimeError(
                    "canonical source membership immutability trigger missing"
                )
        ...
        memberships = conn.execute(
            """SELECT * FROM canonical_message_sources
                WHERE canonical_turn_id = %s
                   OR assistant_canonical_turn_id = %s
                ORDER BY message_id""",
    ...
    def claim_or_verify_conversation(
        ...
        with self.pool.connection() as conn, conn.transaction():
            ...
            observed = str(row["tenant_id"] if row is not None else "")
    ...
    def resolve_or_claim_conversation_for_tenant(
        ...
        """Resolve an alias or claim an unaliased id in one transaction."""
        ...
        with self.pool.connection() as conn, conn.transaction():
            ...
            for _ in range(8):
                # Alias writers take this exact transaction lock before DML.
                # SELECT ... FOR UPDATE cannot protect an absent alias row.
                conn.execute(
                    "SELECT pg_advisory_xact_lock(hashtextextended(%s, 0))",
                    (current,),
                )
                ...
                target = str(row["target_id"] or "") if row is not None else ""
                if not target:
                    break
                followed_alias = True
                if target in seen:
            ...
            if not followed_alias:
                ...
                if owner is None or str(owner["tenant_id"] or "") != tenant:
                    raise PermissionError(
                        "conversation belongs to a different tenant"
                    )
                conn.execute(
                ...
                # Observe the terminal owner in the same transaction without
    ...
    def conversation_belongs_to_tenant(
        ...
        return (
            row is not None
            and str(row["tenant_id"] or "") == str(tenant_id or "")
        )
    ...
    def verify_lifecycle_epoch(
        ...
        verify_epoch(
            ...
            observed=self.get_lifecycle_epoch(conversation_id),
        )
        ...
        with self.pool.connection() as conn:
            row = conn.execute(
                ...
                HAVING ct.turn_group_number >= 0
                   ...
                   AND (
                       (COUNT(*) = 1
                        AND SUM(CASE WHEN btrim(COALESCE(ct.user_content, '')) <> ''
                                     THEN 1 ELSE 0 END) = 1
                        AND SUM(CASE WHEN btrim(COALESCE(ct.assistant_content, '')) <> ''
                                     THEN 1 ELSE 0 END) = 1)
                       OR
                       (COUNT(*) = 2
                        AND SUM(CASE WHEN btrim(COALESCE(ct.user_content, '')) <> ''
                        ...
                        AND SUM(CASE WHEN btrim(COALESCE(ct.assistant_content, '')) <> ''
                                     THEN 1 ELSE 0 END) = 1
                        AND SUM(CASE WHEN btrim(COALESCE(ct.user_content, '')) <> ''
                                           AND btrim(COALESCE(ct.assistant_content, '')) <> ''
                                     THEN 1 ELSE 0 END) = 0)
                       OR
                       (COUNT(*) = 1
                        AND MAX(ct.sort_key) < (
                            SELECT MAX(ct_later.sort_key)
                              FROM canonical_turns ct_later
                             WHERE ct_later.conversation_id = %s
                        )
                        AND (
                            SUM(CASE WHEN btrim(COALESCE(ct.user_content, '')) <> ''
                                     THEN 1 ELSE 0 END)
                            + SUM(CASE WHEN btrim(COALESCE(ct.assistant_content, '')) <> ''
                                      THEN 1 ELSE 0 END)
                        ) = 1)
                   )
                 LIMIT 1
        ...
        with self.pool.connection() as conn:
            with conn.transaction():
                ...
                if (
                    ...
                    return "stale"
                old_phase = str(conversation["phase"])
                ...
                complete = conn.execute(
                    ...
                    SELECT 1
                      FROM canonical_turns
                     WHERE conversation_id = %s AND turn_group_number >= 0
                     GROUP BY turn_group_number
                    HAVING SUM(CASE WHEN tagged_at IS NULL THEN 1 ELSE 0 END) > 0
                       AND (
                           (COUNT(*) = 1
                            AND SUM(CASE WHEN btrim(COALESCE(user_content, '')) <> ''
                                         THEN 1 ELSE 0 END) = 1
                            AND SUM(CASE WHEN btrim(COALESCE(assistant_content, '')) <> ''
                                         THEN 1 ELSE 0 END) = 1)
                           OR
                           (COUNT(*) = 2
                            AND SUM(CASE WHEN btrim(COALESCE(user_content, '')) <> ''
                            ...
                            AND SUM(CASE WHEN btrim(COALESCE(assistant_content, '')) <> ''
                                         THEN 1 ELSE 0 END) = 1
                           AND SUM(CASE WHEN btrim(COALESCE(user_content, '')) <> ''
                                               AND btrim(COALESCE(assistant_content, '')) <> ''
                                         THEN 1 ELSE 0 END) = 0)
                           OR
                           (COUNT(*) = 1
                            AND MAX(sort_key) < (
                            ...
                            AND (
                                SUM(CASE WHEN btrim(COALESCE(user_content, '')) <> ''
                                         THEN 1 ELSE 0 END)
                                + SUM(CASE WHEN btrim(COALESCE(assistant_content, '')) <> ''
                                          THEN 1 ELSE 0 END)
                            ) = 1)
                       )
                     LIMIT 1
                ...
                if transitioned:
                    cur = conn.execute(
                        """
                        UPDATE conversations
                           SET phase = 'ingesting', updated_at = %s
                ...
                conn.execute(
                    ...
                    DO UPDATE SET raw_payload_entries =
                        GREATEST(ingestion_episode.raw_payload_entries,
                                 EXCLUDED.raw_payload_entries)
                    """,
                    (
                        uuid.uuid4(), conversation_id, lifecycle_epoch,
            ...
            with self._alias_post_commit_scope_cm(conn) as post_commit_scope, \
                ...
                for cid in sorted({source_conversation_id, target_conversation_id}):
                    conn.execute(
                        ...
                        """,
                        (cid, _dt_to_str(datetime.now(timezone.utc))),
                    )
                    conn.execute(
                        """
                        SELECT 1 FROM conversation_lifecycle
        ...
        with self.pool.connection() as conn:
            cur = conn.execute(
                ...
                (
                    ...
                    worker_id, conversation_id, conversation_id,
                ),
            )
            return cur.rowcount == 1
                    ...
                    SELECT pending_raw_payload_entries, lifecycle_epoch,
                           ...
                           EXISTS (
                             ...
                             HAVING SUM(CASE WHEN ct.tagged_at IS NULL THEN 1 ELSE 0 END) > 0
                                AND (
                                    (COUNT(*) = 1
                                     AND SUM(CASE WHEN btrim(COALESCE(ct.user_content, '')) <> '' THEN 1 ELSE 0 END) = 1
                                     AND SUM(CASE WHEN btrim(COALESCE(ct.assistant_content, '')) <> '' THEN 1 ELSE 0 END) = 1)
                                    OR
                                    (COUNT(*) = 2
                                     AND SUM(CASE WHEN btrim(COALESCE(ct.user_content, '')) <> '' THEN 1 ELSE 0 END) = 1
                                     AND SUM(CASE WHEN btrim(COALESCE(ct.assistant_content, '')) <> '' THEN 1 ELSE 0 END) = 1
    ...
    def save_conversation_alias(
        ...
        *,
        ...
        # Callback fires after the explicit lock-bearing transaction commits.
        if on_committed is not None:
            event = self._build_alias_created_event(alias_id, target_id)
            on_committed(event)
    ...
    def save_canonical_turn(
        ...
        with self._canonical_write_transaction() as conn:
            ...
            if source_claim:
                self._attest_canonical_user_source_on_connection(
                    conn,
                    CanonicalTurnRow(
                        conversation_id=conversation_id,
                        canonical_turn_id=str(canonical_turn_id),
    ...
    def save_attested_canonical_pair(
        ...
        *,
        ...
        reconcile_scope = self._reconcile_generation_scope.get()
        if reconcile_scope is None:
        ...
        with self.pool.connection() as conn, conn.transaction():
            token = self._attested_pair_connection.set(conn)
            try:
                from ..core.lifecycle_epoch import verify_epoch
                # ``conversation_reconcile`` already owns the matching
                # conversation_lifecycle row lock before this transaction is
                # opened. Lock the progress row second, matching delete's
                # lifecycle -> conversations order without trying to
                # reacquire our own outer lock through another pool checkout.
                ...
                verify_epoch(
                    ...
                    expected=int(expected_lifecycle_epoch),
                    observed=(
                        int(epoch_row["lifecycle_epoch"])
                        if epoch_row is not None else -1
                    ),
                )
                if str(epoch_row["phase"]) in {"deleted", "merged"}:
                    raise CanonicalSourceConflict(
                ...
                self._attested_pair_connection.reset(token)
            ...
            rows = conn.execute(
                """SELECT ct.*,
                    ...
                    WHERE ct.conversation_id = %s
                      ...
                      AND cms.message_id = %s
                    LIMIT 2""",
                (
                    row.conversation_id, claim["agent_scope_id"],
                    claim["platform"], claim["account_id"],
                    claim["message_id"],
                ),
            ).fetchall()
        if not rows:
    ...
    def attest_canonical_user_source(
        ...
        *,
        ...
        """Atomically bind a live adapter source to one physical user row."""
        with self.pool.connection() as conn, conn.transaction():
            self._attest_canonical_user_source_on_connection(
                conn, row, observed_at=observed_at,
                assistant_row=assistant_row,
            )
        return

    def _attest_canonical_user_source_on_connection(
        self,
        conn,
        row: "CanonicalTurnRow",
        *,
        ...
        """Connection-local source insert used by canonical write transaction."""
            ...
            cur = conn.execute(
                ...
                UPDATE canonical_turns
                   SET primary_tag = %s,
                       tags_json = %s,
                       session_date = CASE
                           WHEN btrim(COALESCE(session_date, '')) = ''
                            AND btrim(%s) <> '' THEN %s
                           ELSE session_date
                       END,
                       fact_signals_json = %s,
                 ...
                 WHERE canonical_turn_id = %s
                   AND conversation_id = %s
                   AND turn_hash = %s
                   AND (NOT %s OR tagged_at IS NULL)
                   AND EXISTS (
                       SELECT 1 FROM conversations c
                        WHERE c.conversation_id = %s
                          AND c.lifecycle_epoch = %s
        ...
        *,
        ...
        with self.pool.connection() as conn:
            with conn.transaction():
                ...
                observed = {
                    str(row["canonical_turn_id"]): str(row["turn_hash"] or "")
                    for row in rows
                }
                user_count = sum(
                    bool(str(row["user_content"] or "").strip()) for row in rows
                )
                assistant_count = sum(
                    bool(str(row["assistant_content"] or "").strip()) for row in rows
                )
                combined_count = sum(
                    bool(str(row["user_content"] or "").strip())
                    and bool(str(row["assistant_content"] or "").strip())
                    for row in rows
                )
                valid_shape = (
                    len(rows) == 1
                    ...
                    # payload entries mint a group holding one row with
                    # exactly one half. The strict payload mapper proves
                    # per-message hash identity before reaching this CAS;
                    # refusing the shape here would let admission mint
                    # groups that tagging can never process.
                    len(rows) == 1
                    and combined_count == 0
                ...
                if (
                    ...
                    or not any(row["tagged_at"] is None for row in rows)
                ):
                    return False
                cur = conn.execute(
                    """
                    UPDATE canonical_turns
                       SET primary_tag = %s, tags_json = %s, session_date = %s,
                           fact_signals_json = %s, code_refs_json = %s,
                           tagged_at = %s, updated_at = %s
                     WHERE conversation_id = %s
                       AND turn_group_number = %s
                       AND canonical_turn_id = ANY(%s)
                    """,
                ...
                return int(cur.rowcount or 0) == len(rows)
        ...
        with self.pool.connection() as conn:
            cur = conn.execute(
                ...
                UPDATE canonical_turns
                   SET turn_hash = %s,
                       hash_version = %s,
                       normalized_user_text = %s,
                       normalized_assistant_text = %s,
                       updated_at = %s
                 WHERE canonical_turn_id = %s
                   AND conversation_id = %s
                   AND btrim(COALESCE(turn_hash, '')) = ''
                   AND COALESCE(user_content, '') = %s
                   AND COALESCE(assistant_content, '') = %s
                   AND EXISTS (
                       SELECT 1 FROM conversations c
                        WHERE c.conversation_id = %s
                          AND c.lifecycle_epoch = %s

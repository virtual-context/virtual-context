...
_SUMMARY_FIELDS_DEFAULTS: dict[str, object] = {
    ...
    "skipped_scaffolding_entry_count": 0,
...
class ProxyMetrics:
    ...
    def _capture_matches(
        ...
        req_turn_id = req.get("turn_id", "") or ""
        if turn_id:
            if req_turn_id != turn_id:
                return False
        elif req.get("turn") != turn:
            return False
    ...
    def _find_capture(
        ...
        for req in reversed(self._request_bodies):
            if self._capture_matches(req, turn, conversation_id, turn_id):
                return req
        return None
        ...
        *,
        ...
        with self._lock:
            ...
            payload = {
                ...
                "ingestible_entry_count": int((payload_accounting or {}).get("ingestible_entry_count", 0) or 0),
                "skipped_scaffolding_entry_count": int((payload_accounting or {}).get("skipped_scaffolding_entry_count", 0) or 0),
                "skipped_non_chat_entry_count": int((payload_accounting or {}).get("skipped_non_chat_entry_count", 0) or 0),
                "skipped_tool_result_entry_count": int((payload_accounting or {}).get("skipped_tool_result_entry_count", 0) or 0),
                "skipped_assistant_tool_only_entry_count": int((payload_accounting or {}).get("skipped_assistant_tool_only_entry_count", 0) or 0),
                "skipped_empty_chat_entry_count": int((payload_accounting or {}).get("skipped_empty_chat_entry_count", 0) or 0),
            ...
            if conv_id:
                ...
                self._persist_capture(turn, turn_id=turn_id or None)
    ...
    def capture_enriched(
        ...
        *,
        ...
        with self._lock:
            ...
            req = self._find_capture(turn, conv_id, turn_id)
            if req is not None:
                req["enriched"] = body
                if conv_id:
                    self._persist_capture(
                    ...
                    self._persist_capture(turn, turn_id=turn_id)

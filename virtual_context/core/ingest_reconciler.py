...
def _row_source_identity(
    row: "CanonicalTurnRow",
) -> tuple[str, str] | None:
    """Return the durable user-message identity used across ingest paths.

    A platform message id is not globally unique after VCMERGE, so it is an
    identity only together with its proved audience. Assistant rows and rows
    whose audience is unknown deliberately have no source identity.
    """
    if not (row.user_content or "").strip():
        return None
    source_id = (row.source_message_id or "").strip()
    audience = (row.audience_conversation_id or "").strip()
    if not source_id or not audience:
        return None
...
class IngestReconciler:
    ...
    def ingest_observation(
        ...
        *,
        ...
        """Persist one ambient human message without inventing an assistant.

        This is deliberately separate from :meth:`ingest_single` and
        :meth:`ingest_batch`.  Those paths reconcile provider transcripts and
        therefore reason about user/assistant pairs.  An ambient guild message
        is a complete observation in its own right: storing a synthetic
        assistant half would corrupt both authorship and later actor-card
        evidence.

        Idempotency is the stable platform ``source_message_id`` under the
        proved audience.  The lookup and append run inside
        ``conversation_reconcile``; PostgreSQL implements that surface with a
        row-level lifecycle lock, so concurrent workers serialize without a
        schema-wide uniqueness rule that would break legitimate VCMERGE
        collisions.

        Observations receive the deterministic ``_general`` tag immediately.
        That makes them complete for ingestion accounting without putting an
        LLM call on every guild message.  They are still embedded here, surface
        in the protected DB window immediately, and later pass through the
        normal compaction/fact/actor-card admission pipeline.
        """
        from .lifecycle_epoch import LifecycleEpochMismatch, verify_epoch

        body = (content or "").strip()
        source_id = (source_message_id or "").strip()
        audience = (audience_conversation_id or "").strip()
        channel_id = (origin_channel_id or "").strip()
        actor_id = (sender_actor_id or "").strip()
        if not body:
            raise ValueError("observation content must be non-empty")
        if not source_id:
            raise ValueError("observation source_message_id must be non-empty")
        if not audience:
            raise ValueError(
                "observation audience_conversation_id must be proved"
            )
        if not channel_id:
            raise ValueError("observation origin_channel_id must be non-empty")
        if not actor_id:
            raise ValueError("observation sender_actor_id must be non-empty")
        ...
        if stamp:
            try:
                ...
                raise ValueError("observation observed_at must be ISO-8601") from exc
        ...
        with self._conversation_merge_lock(conversation_id):
            ...
            candidates = [
                ...
                and (row.audience_conversation_id or "").strip() == audience
                and (row.user_content or "").strip()
            ]

            # A merge can legitimately place duplicate opaque platform ids
            # under one owner.  Ambiguity is a conflict, never permission to
            # select one row or append a third.
            if len(candidates) > 1:
                logger.error(
                    "OBSERVATION_SOURCE_CONFLICT conv=%s audience=%s "
                    "source_message_id=%s candidates=%d",
                    conversation_id[:12],
                    audience[:24],
                    source_id[:32],
                    len(candidates),
                ...
                return CanonicalIngestResult(
                    merge_mode="observation_source_conflict",
                    turns_written=0,
                    turns_matched=0,
                    turns_appended=0,
                    turns_prepended=0,
            ...
            if candidates:
                stored = candidates[0]
                stored_channel = (stored.origin_channel_id or "").strip()
                stored_actor = (stored.sender_actor_id or "").strip()
                if (
                    (stored_channel and stored_channel != channel_id)
                    or (stored_actor and stored_actor != actor_id)
                ):
                    logger.error(
                        "OBSERVATION_PROVENANCE_CONFLICT conv=%s source=%s "
                        "stored_channel=%s incoming_channel=%s "
                        "stored_actor=%s incoming_actor=%s",
                        conversation_id[:12],
                        source_id[:32],
                        stored_channel,
                        channel_id,
                    ...
                    return CanonicalIngestResult(
                        merge_mode="observation_provenance_conflict",
                        turns_written=0,
                        turns_matched=0,
                        turns_appended=0,
                        turns_prepended=0,
                ...
                if stored.canonical_turn_id:
                    self._upgrade_empty_senders(
                        conversation_id,
                        {stored.canonical_turn_id: (sender or "").strip()},
                        expected_lifecycle_epoch=expected_lifecycle_epoch,
                    )
                    self._upgrade_empty_channels(
                        conversation_id,
                        {
                            stored.canonical_turn_id: (
                                channel_id,
                                (origin_channel_label or "").strip(),
                            )
                        },
                        expected_lifecycle_epoch=expected_lifecycle_epoch,
                    )
                    self._upgrade_empty_actors(
                        conversation_id,
                        {stored.canonical_turn_id: actor_id},
                        expected_lifecycle_epoch=expected_lifecycle_epoch,
                    )
                    duplicate_edge = {
                        ...
                        "reply_target_body": (reply_target_body or "").strip(),
                        "reply_attribution_version": (
                            REPLY_ATTRIBUTION_VERSION
                            if (
                                (reply_target_message_id or "").strip()
                                or (reply_subject_actor_id or "").strip()
                                or (reply_subject_label or "").strip()
                                or (reply_target_body or "").strip()
                            )
                            else 0
                        ),
                    ...
                    self._upgrade_empty_reply_roles(
                        conversation_id,
                        {stored.canonical_turn_id: duplicate_edge},
                        expected_lifecycle_epoch=expected_lifecycle_epoch,
                    )
                self._upsert_observation_actor_profile(
                    conversation_id,
                    actor_id,
                    (sender or "").strip() or stored.sender,
                    seen_at=stamp,
                    expected_lifecycle_epoch=expected_lifecycle_epoch,
                )
                mode = (
                    "observation_duplicate"
                    if (stored.user_content or "").strip() == body
                    else "observation_duplicate_content_changed"
                )
                logger.info(
                    "OBSERVATION_MATCH conv=%s source=%s mode=%s",
                    conversation_id[:12],
                    source_id[:32],
                    mode,
                )
            ...
            reply_present = any(
                (
                    (reply_target_message_id or "").strip(),
                    (reply_subject_actor_id or "").strip(),
                    (reply_subject_label or "").strip(),
                    (reply_target_body or "").strip(),
                )
            )
            row = self._prepare_message_row(
                conversation_id,
                role="user",
                content=body,
                ...
                origin_channel_label=(origin_channel_label or "").strip(),
                ...
                reply_subject_label=(reply_subject_label or "").strip(),
                reply_target_body=(reply_target_body or "").strip(),
                reply_attribution_version=(
                    REPLY_ATTRIBUTION_VERSION if reply_present else 0
                ),
                audience_conversation_id=audience,
                audience_attribution_version=AUDIENCE_ATTRIBUTION_VERSION,
            )
            self._resolve_reply_subjects(conversation_id, [row])
            batch_id = generate_canonical_turn_id()
            row.canonical_turn_id = generate_canonical_turn_id()
            row.turn_number = len(existing)
            existing_groups: list[int] = []
            for candidate in existing:
                try:
                    group = int(candidate.turn_group_number)
                except (TypeError, ValueError):
                    continue
                if group >= 0:
                    existing_groups.append(group)
            row.turn_group_number = max(existing_groups, default=-1) + 1
            row.sort_key = default_sort_key(existing)
            row.source_batch_id = batch_id
            row.tagged_at = utcnow_iso()
            ...
            batch = self._save_batch(
                ...
                first_turn_hash=row.turn_hash,
                last_turn_hash=row.turn_hash,
                turns_matched=0,
                turns_appended=1,
                turns_prepended=0,
                turns_inserted=0,
            ...
            observed_epoch = self._store.get_lifecycle_epoch(conversation_id)
            if observed_epoch != expected_lifecycle_epoch:
            ...
            logger.info(
                "OBSERVATION_APPEND conv=%s source=%s group=%d channel=%s "
                "actor=%s",
                conversation_id[:12],
                source_id[:32],
    ...
    def _upsert_observation_actor_profile(
        self,
        conversation_id: str,
        actor_id: str,
        display_name: str,
        *,
        ...
        try:
            ...
            logger.warning(
                "OBSERVATION_ACTOR_PROFILE_UPSERT_FAILED conv=%s actor=%s",
                conversation_id[:12],
                actor_id[:24],
                exc_info=True,
            )
    ...
    def _ingest_prepared_turns_locked(
        ...
        *,
        ...
        # The observation hook and the normal prepare/ingest path can race.
        # Source identity is the durable server-side backstop: never append a
        # second physical user row for the same proved platform message merely
        # because the two paths normalized its display text differently.
        #
        # Multiple existing rows under one source identity are already
        # corrupted/ambiguous (for example, two pre-fix race winners). Refuse
        # to choose one or append a third. Likewise, contradictory durable
        # channel/actor provenance is a conflict, not an alignment hint.
        existing_by_source: dict[
            tuple[str, str], list[CanonicalTurnRow]
        ] = {}
        for row in existing:
            identity = _row_source_identity(row)
            if identity is not None:
                existing_by_source.setdefault(identity, []).append(row)
        incoming_by_source: dict[
            tuple[str, str], list[CanonicalTurnRow]
        ] = {}
        for row in prepared_turns:
            identity = _row_source_identity(row)
            if identity is not None:
                incoming_by_source.setdefault(identity, []).append(row)
        for identity, incoming_rows in incoming_by_source.items():
            stored_rows = existing_by_source.get(identity, [])
            if len(stored_rows) > 1:
                logger.error(
                    "CANONICAL_SOURCE_IDENTITY_CONFLICT: conv=%s "
                    "source=%s audience=%s existing=%d",
                    conversation_id[:12],
                    identity[0][:32],
                    identity[1][:24],
                    len(stored_rows),
                ...
                return CanonicalIngestResult(
                    merge_mode="source_identity_conflict",
                    turns_written=0,
                    turns_matched=0,
                    turns_appended=0,
                    turns_prepended=0,
            ...
            if len(stored_rows) != 1 or len(incoming_rows) != 1:
                continue
            stored = stored_rows[0]
            incoming = incoming_rows[0]
            stored_channel = (stored.origin_channel_id or "").strip()
            incoming_channel = (incoming.origin_channel_id or "").strip()
            stored_actor = (stored.sender_actor_id or "").strip()
            incoming_actor = (incoming.sender_actor_id or "").strip()
            if (
                stored_channel
                and incoming_channel
                and stored_channel != incoming_channel
            ) or (
                stored_actor
                and incoming_actor
                and stored_actor != incoming_actor
            ):
                logger.error(
                    "CANONICAL_SOURCE_PROVENANCE_CONFLICT: conv=%s "
                    "source=%s audience=%s stored_channel=%s "
                    "incoming_channel=%s stored_actor=%s incoming_actor=%s",
                    conversation_id[:12],
                    identity[0][:32],
                    identity[1][:24],
                    stored_channel,
                ...
                return CanonicalIngestResult(
                    merge_mode="source_provenance_conflict",
                    turns_written=0,
                    turns_matched=0,
                    turns_appended=0,
                    turns_prepended=0,
        ...
        *,
        ...
        def _alignment_mode(
            ...
            if incoming_start == 0 and existing_start == 0:
                return (
                    "tail_append"
                    if len(incoming_hashes) > overlap_len
                    else "exact_resend"
                )
            if (
                existing_start == 0
        ...
        def _rows_match(
            existing_row: CanonicalTurnRow,
            incoming_row: CanonicalTurnRow,
        ) -> bool:
            if existing_row.turn_hash == incoming_row.turn_hash:
                return True
            existing_identity = _row_source_identity(existing_row)
            return (
                existing_identity is not None
                and existing_identity == _row_source_identity(incoming_row)
            )
        ...
        # A source identity is stronger than a display-text hash and provides
        # the alignment seed when an ambient observation is later seen through
        # the normal invocation path. Require uniqueness on both sides, then
        # expand across ordinary hash matches (or further exact identities).
        existing_source_positions: dict[
            tuple[str, str], list[int]
        ] = {}
        incoming_source_positions: dict[
            tuple[str, str], list[int]
        ] = {}
        for idx, row in enumerate(existing):
            identity = _row_source_identity(row)
            if identity is not None:
                existing_source_positions.setdefault(identity, []).append(idx)
        for idx, row in enumerate(incoming):
            identity = _row_source_identity(row)
            if identity is not None:
                incoming_source_positions.setdefault(identity, []).append(idx)

        source_best: _Alignment | None = None
        for identity, incoming_positions in incoming_source_positions.items():
            existing_positions = existing_source_positions.get(identity, [])
            if len(existing_positions) != 1 or len(incoming_positions) != 1:
                continue
            existing_seed = existing_positions[0]
            incoming_seed = incoming_positions[0]
            left = 0
            while (
                existing_seed - left - 1 >= 0
                and incoming_seed - left - 1 >= 0
                and _rows_match(
                    existing[existing_seed - left - 1],
                    incoming[incoming_seed - left - 1],
                )
            ):
                left += 1
            right = 1
            while (
                existing_seed + right < len(existing)
                and incoming_seed + right < len(incoming)
                and _rows_match(
                    existing[existing_seed + right],
                    incoming[incoming_seed + right],
                )
            ):
                right += 1
            existing_start = existing_seed - left
            incoming_start = incoming_seed - left
            ...
            if (
                source_best is None
                or candidate.overlap_len > source_best.overlap_len
            ):
